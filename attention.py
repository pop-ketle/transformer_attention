...
class SinmpleAttention(tf.keras.models.Model):
    ...
    def call(self, input, memory):
        ...
        Args:
            input (tf.Tensor): queryのテンソル
            memory (tf.Tensor): queryに情報を与えるmemoryのテンソル
        Returns:
            tf.Tensor
        '''
...
class MultiheadAttention(tf.keras.models.Model):
    ...
    def __init__(self, hidden_dim, head_num, dropout_rate, *args, **kwargs):
        ...
        Args:
            hidden_dum (int): 隠れ層及び出力の次元、head_numの倍数である必要がある
            head_num (int): ヘッドの数
            dropout_rate (float): ドロップアウトする確率
        '''
        super().__init__(*args, **kwargs)
    ...
    def call(self, input, memory, attention_mask, training):
        ...
        Args:
            input (tf.Tensor): queryのテンソル
            memory (tf.Tensor): queryに情報を与えるmemoryのテンソル
            attention_mask (tf.Tensor): attention weightに適用されるmask
                shape = [batch_size, 1, q_length, k_length]
                pad等無視する部分がTrueとなるようなものを指定してください
            training (bool): 学習直推論直のフラグ
        Returns:
            tf.Tensor
        '''
    ...
    def _split_head(self, x):
        ...
        Args:
            x (tf.Tensor): [batch_size, length, hidden_dim]
        Returns:
    ...
    def _combine_head(self, x):
        ...
        Args:
            x (tf.Tensor): [batch_size, head_num, length, hidden_dim//head_num]
        Returns:
